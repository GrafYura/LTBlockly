...
!COMPILED && goog.DEPENDENCIES_ENABLED && (goog.inHtmlDocument_ = function() {
		...
		if (a in this.requiresTranspilation_) return this.requiresTranspilation_[a] ? !0 : !goog.inHtmlDocument_() || "es6" != b || "noModule" in goog.global.document.createElement("script") ?
			!1 : !0;
		...
		this.deferredQueue_ = []
	}, goog.DebugLoader_.prototype.bootstrap =
	function(a, b) {
		function c() {
	...
	function() {}, goog.LoadController.prototype.resume = function() {}, goog.LoadController.prototype.loaded = function() {}, goog.LoadController.prototype.pending = function() {}, goog.LoadController.prototype.registerEs6ModuleExports = function(a, b, c) {}, goog.LoadController.prototype.setModuleState = function(a) {}, goog.LoadController.prototype.clearModuleState = function() {}, goog.LoadController.prototype.defer = function(a) {}, goog.LoadController.prototype.areDepsLoaded = function() {}, goog.Dependency = function(a, b, c, d, e) {
	...
	}, goog.Es6ModuleDependency = function(a, b,
		c, d, e) {
		...
		var e = this;
		if (goog.global.CLOSURE_IMPORT_SCRIPT) b(), this.contents_ && goog.global.CLOSURE_IMPORT_SCRIPT("", this.contents_) ?
			(this.contents_ = null, a.loaded()) : a.pause();
		else {
			...
			if (h || g) a.defer(function() {
				...
				if (f && goog.inHtmlDocument_() && goog.isDocumentLoading_() && !h) {
					...
					k.onreadystatechange = function() {
						"interactive" ==
						k.readyState && (k.onreadystatechange = l, c(), a.resume());
		...
		return this.transpiler.transpile(a,
			this.path)
		...
		for (c = 0; c < b.length; c++) this.idToPath_[b[c]] =
			e;
		this.idToPath_[a] = e
	...
	for (var d = [], e = Object.create(null), f = 0, g; g = c[f]; f++)
		if (!g.isNull()) {
			...
			k ? g.type == Blockly.Events.MOVE && k.index == f - 1 ? (l.newParentId = g.newParentId, l.newInputName = g.newInputName, l.newCoordinate = g.newCoordinate, k.index = f) : g.type == Blockly.Events.CHANGE && g.element == l.element && g.name == l.name ? l.newValue = g.newValue : g.type != Blockly.Events.UI || "click" != g.element ||
				"commentOpen" != l.element && "mutatorOpen" != l.element && "warningOpen" != l.element ? (e[h] = {
					event: g,
					index: 1
	...
	for (var g; g = a.childNodes[e]; e++)
		if (3 != g.nodeType) {
			...
			switch (g.nodeName.toLowerCase()) {
				...
				case "title":
				case "field":
					var index = k.substring(k.length - 1);
					if(c.type == 'csv_google_sprdsht_controller' && k.indexOf('COLUMNDSCR') != -1 && k != 'COLUMNDSCR0')
						dscrpns.push(g.textContent);
					if(!c.getField(k) && c.type == 'event_on_device'){
						if(k.indexOf('COND')!=-1){
							c.appendDummyInput(k)
								...
								.appendField(new Blockly.FieldDropdown([
									...
									[Blockly.Msg["LT_EVENT_DEVICE_CONDITION_SWITCH_FC"], " == 0xFC"]]),k)
								.appendField(new Blockly.FieldNumber(50),'VALUE' + index);
								if(c.getInput('DEVS' + index))
									c.moveInputBefore(k,'DEVS' + index);
								else{
									c.appendStatementInput('DEVS' + index)
										.appendField(Blockly.Msg["LT_CATEGORY_EVENTS_EVENT_DEVICE_CONDITION_TRUE"]);
									c.moveInputBefore('DEVS' + index,'ADDCOND');
									c.moveInputBefore(k,'DEVS' + index);
								}
						}else{
							if(k.indexOf('TIMER_VALUE')!=-1){ 
								c.appendDummyInput('TIMER'+ index)
									.appendField(Blockly.Msg["LT_EVENT_DEVICE_DO_ONCE_IN"])
									.appendField(new Blockly.FieldNumber(10,5), k)
									.appendField(Blockly.Msg["LT_DATA_TIME_SEC"])
									.appendField(new Blockly.FieldCheckbox('TRUE'), 'TIMER'+ index);
								if(c.getInput('DEVS' + index))
									c.moveInputBefore('TIMER'+ index,'DEVS' + index);
							}
						}	
					}else if(!c.getField(k) && c.type == 'variebles_getter'){
						getter_block_edit(c,k);
					}else if(!c.getField(k) && c.type == 'variebles_setter'){
						setter_block_edit(c,'ARRAY_INDEX');
					}else if(!c.getField(k) && c.type == 'univarsal_modbus'){
						add_register_settings.call(c,k.substring(0,k.indexOf('_')));
					}else if(!c.getField(k) && c.type == 'imitation'){
						imitation_edit(c,k);
					}else if(!c.getField(k) && c.type == 'moving_music'){
						if(k.indexOf('SENS') != -1){
							addSens(c,k.substring(k.length - 2, k.length - 1),k.substring(k.length - 1));
							addSensSettings(c,k.substring(k.length - 2, k.length - 1),k.substring(k.length - 1));
						}else if (k.indexOf('VAL') != -1){
							addSensSettings(c,k.substring(k.length - 2, k.length - 1),k.substring(k.length - 1));
							addSensValue.call(c,'SENS' + k.substring(k.length - 2, k.length - 1) + k.substring(k.length - 1));
						}else if (k.indexOf('SPEAKER') != -1){
							moving_music_edit(c,'AREA' + k.substring(k.length - 1));
						}
					}else if(!c.getField(k) && c.type == 'related_actions'){
						related_actions_edit(c,k);
					}else if(!c.getField(k) && c.type == 'save_device_state'){
						addField.call(c,'DEVICE',k,'number',{value:10,min:1,max:30,label:Blockly.Msg["LT_PERIOD"] + ':'});
					}
				...
				case "statement":
					g = c.getInput(k);
					if(!g && c.type == 'csv_google_sprdsht_controller'){
						c.appendValueInput(k)
							.appendField((parseInt(k.substring(k.length - 1)) + 1) + ' column: ')
							.appendField(new Blockly.FieldTextInput(dscrpns[index]), 'COLUMNDSCR'  +k.substring(k.length - 1));
						c.getInput(k).setCheck('Text');
						c.moveInputBefore(k,'ADDCOLUMN');
						g = c.getInput(k);
						++index;
					}else if(!g && c.type == 'event_on_device'){
						if(k.indexOf('DEVS')!=-1){
							c.appendStatementInput('DEVS' + k.substring(k.length - 1))
								.appendField(Blockly.Msg["LT_CATEGORY_EVENTS_EVENT_DEVICE_CONDITION_TRUE"]);
							c.moveInputBefore('DEVS' + k.substring(k.length - 1),'ADDCOND');
						}
						g = c.getInput(k);
					}else if(!g && c.type == 'variebles_setter'){
						setter_block_edit(c,k);
						g = c.getInput(k);
					}else if(!g && c.type == 'variebles_getter'){
						getter_block_edit(c,k);
						g = c.getInput(k);
					}else if(!g){
